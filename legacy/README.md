...
./start-claude-nim.sh --w1
./start-claude-nim.sh --w2
./start-claude-nim.sh --w3

# Start all at once
./start-claude-nim.sh --all

# Show help
./start-claude-nim.sh

Here's the setup script in parts:

**Part 1: Create `setup-claude.sh`**
...
**Part 2: Create `start-claude-nim.sh` - Part A (Functions)**
...
**Part 3: Create `start-claude-nim.sh` - Part B (More Functions)**
...
**Part 4: Create `start-claude-nim.sh` - Part C (Main Logic)**
...
1. Copy Part 1 → save as `setup-claude.sh`
2. Copy Parts 2-4 together → save as `start-claude-nim.sh`
3. Copy Part 5 → append to `start-claude-nim.sh`
4. Copy Part 6 → append to `setup-claude.sh`
5. Run: `bash setup-claude.sh`