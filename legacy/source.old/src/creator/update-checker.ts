...
export function formatUpdateMessage(
  ...
  const validAnnouncements = (updates.announcements || []).filter(
    (a) => a && a.title,
  );
  const validModels = (updates.fastestModels || []).filter(
    (m) => m && m.model,
  );
  const hasNewVersion =
    updates.latestVersion && updates.latestVersion !== currentVersion;
  const hasAnnouncements = validAnnouncements.length > 0;
  const hasModels = validModels.length > 0;
  const hasStats =
  ...
  if (hasAnnouncements) {
    ...
    for (const ann of validAnnouncements.slice(0, 3)) {
      const icon =
        ann.type === "benchmark"
          ? "\u26A1"
          : ann.type === "new-model"
  ...
  if (hasModels) {
    ...
    for (const m of validModels.slice(0, 5)) {
      lines.push(
  ...
  if (hasStats) {
    ...
    lines.push(
      `  ${dim(`${s.totalUsers.toLocaleString()} users \u00B7 ${(s.totalRequests || 0).toLocaleString()} requests${s.topModel ? ` \u00B7 Top: ${s.topModel}` : ""}`)}`,
    );
  }
